...
ResultSet::ResultSet(MYSQL_RES *result, MYSQL_FIELD *fields, vn::u64 rowCount, vn::u32 fieldCount) :
_rowCount(rowCount),
...
PreparedResultSet::PreparedResultSet(MYSQL_STMT* stmt, MYSQL_RES *result, vn::u64 rowCount, vn::u32 fieldCount) :
m_rowCount(rowCount),
m_rowPosition(0),
...
m_length(NULL)
{
	...
	if (mysql_stmt_store_result(m_stmt)) {
		LOG_WARN("%s:mysql_stmt_store_result, cannot bind result from MySQL server. Error: %s", __FUNCTION__, mysql_stmt_error(m_stmt));
		delete[] m_rBind;
		delete[] m_isNull;
		delete[] m_length;
		return;
	}
	LOG_DEBUG("[%u ms]  PreparedResultSet construct", getMSTimeDiff(_s, getMSTime()));
	...
	if (mysql_stmt_bind_result(m_stmt, m_rBind)) {
		LOG_WARN("%s:mysql_stmt_bind_result, cannot bind result from MySQL server. Error: %s", __FUNCTION__, mysql_stmt_error(m_stmt));
		delete[] m_rBind;
		delete[] m_isNull;
		delete[] m_length;
		return;
	...
	m_rows.resize(vn::u32(m_rowCount));
	while (_NextRow()) {
		m_rows[vn::u32(m_rowPosition)] = new Field[m_fieldCount];
		for (vn::u64 fIndex = 0; fIndex < m_fieldCount; ++fIndex) {
			if (!*m_rBind[fIndex].is_null)
				m_rows[vn::u32(m_rowPosition)][fIndex].SetByteValue(m_rBind[fIndex].buffer, m_rBind[fIndex].buffer_length, m_rBind[fIndex].buffer_type, *m_rBind[fIndex].length);
			else
				...
				case MYSQL_TYPE_VAR_STRING:
					m_rows[vn::u32(m_rowPosition)][fIndex].SetByteValue("", m_rBind[fIndex].buffer_length, m_rBind[fIndex].buffer_type, *m_rBind[fIndex].length);
					break;
				default:
					m_rows[vn::u32(m_rowPosition)][fIndex].SetByteValue(nullptr, m_rBind[fIndex].buffer_length, m_rBind[fIndex].buffer_type, *m_rBind[fIndex].length);
				}
		}
		m_rowPosition++;
	}
...
{
    for (vn::u32 i = 0; i < vn::u32(m_rowCount); ++i)
        delete[] m_rows[i];
}
...
bool ResultSet::NextRow()
{
    ...
    for (vn::u32 i = 0; i < _fieldCount; i++)
...
{
    for (vn::u32 i = 0; i < m_fieldCount; ++i)
